...
# Change /etc/containerd/config.toml to use nvidia runtime by default before proceeding
sudo sh -c 'cat append_containerd.toml >> /etc/containerd/config.toml'


echo "Please make sure  /etc/containerd/config.toml is modified to use nvidia runtime by default before proceeding"
